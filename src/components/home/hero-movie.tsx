...
export function HeroMovie({ movie, details, genres }: HeroMovieProps) {
  ...
  return (
    <section className="relative isolate flex min-h-[680px] items-end overflow-hidden bg-[#1a1013] pt-28 sm:min-h-[740px] lg:min-h-[810px]" aria-labelledby="featured-title">
      {backdrop ? (
        <Image src={backdrop} alt="" fill priority quality={84} sizes="100vw" className="-z-20 object-cover object-[58%_top] sm:object-center" />
      ) : (
        <div className="absolute inset-0 -z-20 bg-[radial-gradient(ellipse_at_70%_30%,#4a1e29_0%,#171011_48%,#080808_100%)]" />
      )}
      ...
      <div className="page-shell relative z-10 pb-16 pt-28 sm:pb-20 lg:pb-28">
        <HeroReveal>
      ...
      <div className="absolute bottom-7 right-[max(1.5rem,calc((100%-var(--content-width))/2))] hidden items-center gap-3 text-[10px] font-semibold tracking-[.2em] text-foreground/45 lg:flex" aria-hidden="true"><span className="h-px w-12 bg-foreground/40" /> DISCOVER CINEMA</div>
    </section>
  );
}
